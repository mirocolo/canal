...
public class ESConnection {
	...
	public MappingMetaData getMapping(String indexOrAlias, String type) {
		MappingMetaData mappingMetaData = getMappingByIndex(indexOrAlias, type, false);
		if (mappingMetaData == null) {
			mappingMetaData = getMappingByAlias(indexOrAlias, type);
		}
		return mappingMetaData;
	}


	private MappingMetaData getMappingByIndex(String index, String type, boolean checkIndex) {
		...
		if (mode == ESClientMode.TRANSPORT) {
			...
			try {
				...
				mappingMetaData = response.mappings().get(index).get(type);
			} catch (NullPointerException e) {
				logger.warn("Not found the mapping info of index: " + index);
			} catch (IOException e) {
	...
	private MappingMetaData getMappingByAlias(String alias, String type) {
		MappingMetaData mappingMetaData = null;
		...
		try {
			...
			if (getAliasesResponse != null
					&& getAliasesResponse.getAliases() != null
					&& getAliasesResponse.getAliases().size() == 1) {
				for (String index : getAliasesResponse.getAliases().keySet()) {
					mappingMetaData = getMappingByIndex(index, type, true);
				}
			}
		} catch (IOException e) {
			logger.error("getMapperByAliasError", e);
		}
